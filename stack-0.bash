#!/bin/bash
set -o errexit -o pipefail -o nounset -o noglob

. shell-utils.bash

# IAM is global; use 'us-east-1' for a region and ignore short_region
region='us-east-1'

root="$account_name-cloudformation-lambda-utils-permissions"
json_source='json/lambda-utils-permissions.json'

json=$(mktemp -u --suffix='.json')

cleanup() {
    trap - EXIT
    rm -f *.zip $json $json.swap
}
trap cleanup EXIT

# Process 'json_source' -> 'json'
json_addr='.Resources.CloudFormationLambdaUtilsRole.Properties'
cat $json_source | \
    jq "$json_addr.Tags[0].Value = \"$(date --iso-8601=minutes)\"" > $json

aws --profile wex-$profile --region $region cloudformation \
    $(create_or_update $root-stk)-stack \
    --stack-name $root-stk --template-body file://$json \
    --capabilities CAPABILITY_IAM
